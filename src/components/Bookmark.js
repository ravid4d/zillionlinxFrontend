...
import {  disabledTopLinks, fetchAllTopLinks, fetchCategoryWiseBookmarks, pinBookmark } from "../redux/slices/bookmarkSlice";
import { toast } from "react-toastify";


const Bookmark = ({ item, handleRemoveItem, categoryId, subCategoryId, setId }) => {
  ...
  return (
    <>
      ...
      >
        <span className={`pin-icon w-6 h-6 rounded-full ${item?.pinned ? 'bg-dark-blue' : 'bg-light-blue' } flex flex-wrap items-center justify-center cursor-pointer absolute left-2 top-2`}>
          <label
          ...
          >
            <input
              type="checkbox"
              className="hidden shrink-0 mt-0.5 border-gray-200 rounded text-blue-600 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-800 dark:border-neutral-700 dark:checked:bg-blue-500 dark:checked:border-blue-500 dark:focus:ring-offset-gray-800"
